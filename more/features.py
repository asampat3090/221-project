...
def extractUnigramFeatures(x):
    ...
    @return Counter: feature vector representation of x.
    """    
...
def extractTrigramFeatures(x):
    """
    Extract unigram+bigram+trigram features for a text document $x$. 
...
def extractFourgramFeatures(x):
    """
    Extract unigram+bigram+trigram+fourgram features for a text document $x$. 

    @param string x: represents the contents of a lyrics file.
    @return dict: feature vector representation of x.
    """
    #Clean up string
    x = ''.join(char for char in x if char not in set(string.punctuation))
...
def getDict(minNumberOfSongs, maxNumberOfSongs, trainSongs, gram):
    ...
    @return a list of words that are present in at least minNumOfSongs and less than maxNumOfSongs
    """
    ...
    #Grab words that show up in at least minNumberOfSongs, but not more than maxNumberOfSongs (half)
